<?php
/*!
 * fix incompatible problem of Medoo and Hprose
 * author  zhuanqianfish 2019年11月14日14:17:51
 *
...
class MedooPlus
{
	...
	public function query($query, $map = [])
	{
		...
		return $this->exec($query, $map)->fetch();
	}
	...
	public function create($table, $columns, $options = null)
	{
		...
		return $this->exec("CREATE TABLE IF NOT EXISTS $tableName (" . implode(', ', $stack) . ")$table_option")->fetch();
	}
	...
	public function drop($table)
	{
		...
		return $this->exec("DROP TABLE IF EXISTS $tableName")->fetch();
	}
	...
	public function insert($table, $datas)
	{
        ...
        return $this->exec('INSERT INTO ' . $this->tableQuote($table) . ' (' . implode(', ', $fields) . ') VALUES ' . implode(', ', $stack), $map)->fetch();
	...
	public function update($table, $data, $where = null)
	{
		...
		return $this->exec('UPDATE ' . $this->tableQuote($table) . ' SET ' . implode(', ', $fields) . $this->whereClause($where, $map), $map)->fetch();
	}
	...
	public function delete($table, $where)
	{
		...
		return $this->exec('DELETE FROM ' . $this->tableQuote($table) . $this->whereClause($where, $map), $map)->fetch();
	}
	...
	public function replace($table, $columns, $where = null)
	{
		...
		if (!empty($stack))
		{
			return $this->exec('UPDATE ' . $this->tableQuote($table) . ' SET ' . implode(', ', $stack) . $this->whereClause($where, $map), $map)->fetch();
		}
	...
	public function has($table, $join, $where = null)
	{
		...
		if ($this->type === 'mssql')
		{
			$query = $this->exec($this->selectContext($table, $map, $join, $column, $where, MedooPlus::raw('TOP 1 1')), $map);
		}
		else
		{
